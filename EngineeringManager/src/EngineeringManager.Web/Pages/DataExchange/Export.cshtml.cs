    ...
    IProjectService projectService) : PageModel
{
    ...
    public async Task<IActionResult> OnPostExportAsync(CancellationToken cancellationToken)
    {
        var result = await exportService.ExportAsync(
            new ExportRequest(Dataset, UserId(), SelectedFields, CutoffDate, CanViewSensitiveData: CanManage),
            cancellationToken);
    ...
    public async Task<IActionResult> OnPostExportProjectWorkbookAsync(CancellationToken cancellationToken)
    {
        ...
        var query = new ProjectListQuery(
            ...
            ProjectMinimumCurrentAmount,
            ProjectMaximumCurrentAmount,
            "ProjectNumber",
            true,
    ...
    public async Task<IActionResult> OnPostSaveTemplateAsync(CancellationToken cancellationToken)
    {
        await exportService.SaveTemplateAsync(
            new SaveExportTemplateRequest(
                UserId(),
                TemplateName ?? string.Empty,
                Dataset,
                SharedTemplate ? ExportTemplateScope.Shared : ExportTemplateScope.Personal,
                SelectedFields,
                CutoffDate,
                CanManage),
            cancellationToken);
    ...
    private async Task LoadAsync(CancellationToken cancellationToken)
    {
        ...
        var projectQuery = new ProjectListQuery(
            ProjectSearch,
            ProjectStages,
            ProjectLegalEntityId,
            ProjectResponsibleUserId,
    ...
    {
        var canAccessAll = User.IsInRole(SystemRoles.SystemAdministrator)
