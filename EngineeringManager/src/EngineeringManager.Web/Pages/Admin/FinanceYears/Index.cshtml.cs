...
namespace EngineeringManager.Web.Pages.Admin.FinanceYears;
...
public sealed class IndexModel(IFinanceBusinessYearService service, ApplicationDbContext db) : PageModel
{
    ...
    public async Task<IActionResult> OnPostCreateAsync(CancellationToken token)
    {
        try
        {
            ...
            TempData["Success"] = "财务年度已创建。";
        }
        catch (Exception exception) when (exception is ArgumentException or InvalidOperationException) { TempData["Error"] = exception.Message; }
    ...
    public async Task<IActionResult> OnPostDeleteAsync(CancellationToken token)
    {
        try
        {
            ...
            TempData["Success"] = "财务年度已删除。";
        }
        catch (Exception exception) when (exception is ArgumentException or InvalidOperationException or KeyNotFoundException) { TempData["Error"] = exception.Message; }
        return RedirectToPage();
    }
}
