...
- Historical data backfills must run through an explicit maintenance command, create a SQL Server full backup before any write, apply changes in one transaction, update only empty fields unless the request explicitly authorizes replacement, and persist a JSON audit result. Date backfills must require a complete year and must leave ambiguous or out-of-order candidates for manual review; rerunning the command must be idempotent.
