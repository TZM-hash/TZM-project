...
public sealed class PayrollWorkspaceFilterTests
{
    ...
    public async Task CompanyAndPersonalScopesFilterListAndSummaryFromTheSameBatchSet()
    {
        ...
        var firstCompanyConfirmed = CreateItem(firstCompany.Id, PayrollFundingSource.CompanyAccount, PayrollBatchStatus.Confirmed, 100m, 60m, 40m);
        ...
        var employee = new Employee { EmployeeNumber = "PAY-LIST-E", Name = "名单员工" };
        ...
        var worker = new ConstructionWorker { Name = "名单班组人员" };
        ...
        fixture.Db.AddRange(employee, crew, worker, batch);
        await fixture.Db.SaveChangesAsync();

        var companyModel = CreateModel(fixture, $"company:{firstCompany.Id}", PayrollBatchStatus.Confirmed);
        ...
        companyModel.Overview.ActualAmount.Should().Be(100m);
        companyModel.Overview.EmployeeAmount.Should().Be(60m);
        ...
        breakdown.TotalCount.Should().Be(2);
    ...
    private static IndexModel CreateModel(PageFixture fixture, string? scope, PayrollBatchStatus? status)
    {
